...
export async function updateUserAvatarAction(url: string) {
  const searchUserAvatarUrl = await getProfile();

  const user_url = searchUserAvatarUrl.map((user: any) => user.avatar_url)[0];
  const newUrl = user_url.substring(user_url.lastIndexOf("/") + 1);
  if (user_url !== null) {
    try {
